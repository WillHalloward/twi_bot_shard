...
class CreatorLinks(commands.Cog, name="Creator"):
    ...
    async def cog_load(self) -> None:
        self.links_cache = await self.bot.db.fetch("SELECT title, user_id FROM creator_links")
    ...
    async def creator_link_get(self, interaction: discord.Interaction, creator: discord.User = None):
        ...
        try:
            query_r = await self.bot.db.fetch("SELECT * FROM creator_links WHERE user_id = $1 ORDER BY weight DESC",creator.id)
            if query_r:
    ...
    async def creator_link_add(self, interaction: discord.Interaction, title: str, link: str, nsfw: bool = False, weight: int = 0, feature: bool = True):
        try:
            await self.bot.db.execute("INSERT INTO creator_links (user_id, title, link, nsfw, weight, feature) VALUES ($1, $2, $3,$4, $5, $6)",
    ...
    async def creator_link_remove(self, interaction: discord.Interaction, title: str):
        try:
            await self.bot.db.execute("DELETE FROM creator_links WHERE user_id = $1 AND lower(title) = lower($2)",
    ...
    async def creator_link_edit(self, interaction: discord.Interaction, title: str, link: str, nsfw: bool = False, weight: int = 0, feature: bool = True):
        try:
            await self.bot.db.execute("UPDATE creator_links SET link = $1, nsfw = $2, weight = $5, feature = $6, last_changed = now() WHERE user_id = $3 AND lower(title) = lower($4)",
