...
class ExampleCog(commands.Cog, name="Example"):
    ...
    async def example_fetch(self, interaction: discord.Interaction, limit: int = 10):
        ...
        try:
            ...
            results = await self.bot.db.fetch(
                "SELECT * FROM example_table ORDER BY value DESC LIMIT $1",
                limit
            )
    ...
    async def example_update(self, interaction: discord.Interaction, name: str, value: int):
        """Example command demonstrating update operations with error handling."""
        try:
            # First, check if the record exists
            exists = await self.bot.db.fetchval(
                "SELECT EXISTS(SELECT 1 FROM example_table WHERE name = $1)",
                name
            )
            ...
            if exists:
                ...
                await self.bot.db.execute(
                    "UPDATE example_table SET value = $1 WHERE name = $2",
                    value, name
                )
                ...
                await interaction.response.send_message(f"Created new record {name} with value {value}.")
        except Exception as e:
