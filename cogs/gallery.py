...
class GalleryCog(commands.Cog, name="Gallery & Mementos"):
    ...
    async def repost(
        ...
        if repost_type:
            ...
            if 1 in repost_type:
                embed.add_field(
                    name="Attachment",
                    value=f"I found {len(message.attachments)} attachments",
                    inline=False,
                )
            ...
            if 2 in repost_type:
                embed.add_field(
                    name="AO3 Link",
                    value=f"I found an AO3 link: {re.search(ao3_pattern, message.content).group(0)}",
                    inline=False,
                )
            ...
            if 3 in repost_type:
                embed.add_field(
                    name="Twitter Link",
                    value=f"I found a Twitter link: {re.search(twitter_pattern, message.content).group(0)}",
                    inline=False,
                )
            ...
            if 4 in repost_type:
                embed.add_field(
                    name="Instagram Link",
                    value=f"I found an Instagram link: {re.search(instagram_pattern, message.content).group(0)}",
                    inline=False,
                )
            ...
            if 5 in repost_type:
                embed.add_field(
                    name="Text",
                    value="I can also repost the text in the message.",
                    inline=False,
                )
            if 6 in repost_type:
                embed.add_field(
                    name="Discord File",
                    value=f"I found a discord file: {re.search(discord_file_pattern, message.content).group(0)}",
                    inline=False,
                )
                view.discord_file.disabled = False
            embed.add_field(
                name="Please select the type of repost you want to do",
                value="If you want to repost multiple types, you will have to do it one by one",
                inline=False,
            )
                ...
                match view.repost_choice:
                    ...
                    case _:  # default
                        await interaction.response.send_message(
                            "Something appears to have gone wrong", ephemeral=True
                        )
            else:
                try:
                    await interaction.delete_original_response()
        ...
        if supported:
            ...
            if not await menu.wait() and menu.channel_select.values:
                ...
                if type(repost_channel) != discord.channel.ForumChannel:
                        ...
                        if attachment.content_type.startswith("image"):
                            ...
                            if x == 4:
                                try:
                                    ...
                                    await interaction.response.send_message(
                                        "I could not send the embeds to that channel",
                                        ephemeral=True,
                                    )
                                except ValueError:
                                    await interaction.response.send_message(
                    ...
                    await repost_channel.create_thread(
                        name=menu.title_item, embed=embed, files=list_of_files
                    )
            else:
                await interaction.delete_original_response()
        else:
            ...
            await interaction.response.send_message(
                f"I could not find a supported attachment in that message. Attachment types: {', '.join([attachment.content_type for attachment in message.attachments])}",
                ephemeral=True,
            )
    ...
    async def repost_discord_file(
        ...
        if not await menu.wait() and menu.channel_select.values:
            ...
            for image in sorted(os.listdir(f"{interaction.guild_id}_temp_files")):
                ...
                mime_type, _ = mimetypes.guess_type(f"{interaction.guild_id}_temp_files/{image}")
    ...
    async def set_repost(
        ...
        if existing:
            ...
            await interaction.response.send_message(
                f"Removed {channel.mention} from repost channels", ephemeral=True
            )
        else:
            # Add new channel
            ...
            await interaction.response.send_message(
                f"Added {channel.mention} to repost channels", ephemeral=True
            )
